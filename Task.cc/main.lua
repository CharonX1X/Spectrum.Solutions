...
-- loadstring
local ESP1 = loadstring(game:HttpGet("https://kiriot22.com/releases/ESP.lua"))()
ESP1.Players = false
ESP1.Boxes = false
ESP1.Names = true
ESP1:Toggle(true)
...
ESP1:AddObjectListener(Workspace.RagDoll, { -- Object Path, For example: Workspace.ThisFolder
    Name = "LowerTorso", --Object name inside of the path, for example: Workspace.ThisFolder.Item_1
    CustomName = "Body", -- Name you want to be displayed
    Color = Color3.fromRGB(255,255,255), -- Color
    IsEnabled = "Tracked" -- Any name, has to be the same as the last line: ESP.TheNameYouWant
})

if ESP1.Tracked then
    Workspace.RagDoll.LowerTorso.ChildAdded:Connect(function(child)
        ESPItem1:AddObject(child)
    end)
end
...
ThemeManager:SetFolder('Taskcc')
SaveManager:SetFolder('Taskcc/specific-game')
...
Credits:AddLabel('Dontaii#1380 ~ Founder')
Credits:AddLabel('scaleycat#0001 ~ Founder')
...
local ItemESPWindow = Tabs.Visuals:AddRightGroupbox('Map Visuals')

ItemESPWindow:AddToggle('Enabled1', {
    Text = 'Enabled',
    Default = false, -- Default value (true / false)
})

Toggles.Enabled1:OnChanged(function(state)
ESP1.Names = state
end)

ItemESPWindow:AddToggle('Item1EspToggle', {
    Text = 'Body Esp',
    Default = false, -- Default value (true / false)
})

Toggles.Item1EspToggle:OnChanged(function(state)
ESP1.Tracked = state
end)
